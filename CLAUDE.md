...
`content/` に10本（公開済み）。残り1本は上記のとおり 8/28 に自動公開予約中。

最新は **Coursera vs edX in 2026**（`coursera-vs-edx`, 8/25公開）。
課金モデルの違い（Coursera=完了まで月額課金 / edX=買い切り）を軸に、
学習ペースが不規則な人にはedXが構造的に安いという結論を出した記事。
他サイトが書いていないedXの所有権変遷（Harvard/MITの非営利 → 2021年に2Uが$800Mで買収 →
2024年7月Chapter 11 → 同年9月に投資会社所有の非公開企業として再建）も事実確認のうえ記載している。
...
- 生成済み33枚（`pinterest-pins/output/`）。文言は `pinterest-pins/generate_pins.py` の `PINS` が原本
- **投稿済み27枚**
- **未投稿6枚**:
  - `coursera-edx-01/02/03` → 記事は公開済みなので**いつでも投稿できる**
  - `coursera-certs-01/02/03` → 記事10本目が公開される **8/28以降**に投稿する
...
**既知の不具合**: 8/25に9枚を連続投稿した直後、ピン作成ツール（`/pin-creation-tool/`）の
本体が読み込まれなくなった（ヘッダーだけ表示され40秒待っても空白）。
連続投稿後の一時的なものと思われるので、詰まったら時間を置いて再試行する。
...
1. **Pinterest `coursera-edx` 3枚を投稿**（記事は公開済み。ツールが復旧していれば即できる）
2. **記事12本目以降を書く**（週2〜3本ペース。Courseraクラスターを厚くするのが有効）
...
- Coursera Free Courses（"free"は検索ボリュームが大きくトラフィック獲得向き）
